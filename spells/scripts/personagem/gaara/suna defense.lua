...
doSendMagicEffect(position1, 847) -- Pause
end

local function effect02(position2)
doSendMagicEffect(position2, 848) -- Effect
end

local function effect03(position3)
doSendMagicEffect(position3, 849) -- Broken
end
...
if isInRange(getCreaturePosition(cid), from1, to1, from2, to2, from3, to3, from4, to4, from5, to5, from6, to6, from7, to7, from8, to8, from9, to9, from10, to10, from11, to11, from12, to12, from13, to13, from14, to14, from15, to15) then   doPlayerSendTextMessage(cid, 27, "voce nao pode usar Suna no Yoroi aqui!")
   return doSendMagicEffect(getCreaturePosition(cid), 2)
      ...
      local position1 = {x=getThingPosition(cid).x+1, y=getThingPosition(cid).y+1, z=getThingPosition(cid).z, config.duration * 1000, false}
...
-- doCreatureSay(cid,"Suna no Yoroi", 19) 
