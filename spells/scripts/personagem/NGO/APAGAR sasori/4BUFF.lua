...
local function magicEffectSASORI(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, storage2) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
local waittime = 60
local storage = 100062
local storage2 = 100063
...
if getPlayerStorageValue(cid, storage2) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffectSASORI, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
    setPlayerStorageValue(cid, storage2,1) -- storage verifica transformado, quando = 1 player esta transformado.
