...
local waittime = 59 -- Tempo de exhaustion
...
return false
        ...
        for n = 1, maxSummons do
                ...
                local clone = doCreateMonster("Sasori Marionete", getThingPos(cid), false)
                if isCreature(clone) then
                        doTeleportThing(clone, getThingPos(cid), false)
                        doConvinceCreature(cid, clone)
                        setCreatureMaxHealth(clone, getCreatureMaxHealth(cid)*2)
                        doCreatureAddHealth(clone, getCreatureHealth(cid)*2)
