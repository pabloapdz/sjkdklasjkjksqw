...
setCombatFormula(combat1, COMBAT_FORMULA_LEVELMAGIC, -6.5, -1750, -8.5, -2250)
...
doSendMagicEffect(targetpos, 1101)
doCombat(parameters.cid, parameters.combat1, parameters.var)
end

local combat2 = createCombatObject()
setCombatParam(combat2, COMBAT_PARAM_TYPE, COMBAT_PHYSICALDAMAGE)
setCombatFormula(combat2, COMBAT_FORMULA_LEVELMAGIC, -6.5, -1750, -8.5, -2250)

local function onCastSpell2(parameters, target)
--if not isPlayer(parameters.cid) then return true end
--if not isCreature(target) then return true end
local targetpos = { x=getCreaturePosition(target).x + 1, y=getCreaturePosition(target).y + 1, z=getCreaturePosition(target).z }
doSendMagicEffect(targetpos, 1102)
doCombat(parameters.cid, parameters.combat2, parameters.var)
end

function onCastSpell(cid, var)
local waittime = 1.5 -- Tempo de exhaustion
local storage =100202
...
--doCreatureSay(cid, "Konoha Kaigansho", TALKTYPE_MONSTER)
