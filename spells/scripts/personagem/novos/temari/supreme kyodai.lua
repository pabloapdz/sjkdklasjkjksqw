...
function magicEffect1133(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, 156652) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
if getPlayerStorageValue(cid, 156652) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    doCombat(cid, combat, var)
	doPlayerSendTextMessage(cid,27,'+UP Buff.') 
    doPlayerSay(cid, 'Im feeling better!', TALKTYPE_ORANGE_1)
    tempo2 = 0
    while (tempo2 ~= (tempo*2000)) do
        addEvent(magicEffect1133, tempo2, tempo2, tempo*2000, cid)
        tempo2 = tempo2 + 500
    end
    setPlayerStorageValue(cid, 156652,1) -- storage verifica transformado, quando = 1 player esta transformado.
