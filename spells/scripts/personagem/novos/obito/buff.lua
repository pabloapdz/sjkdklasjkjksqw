...
local effect = {887} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function magicEffect11g27(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, 111211) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
if getPlayerStorageValue(cid, 111211) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    setPlayerStorageValue(cid, 111211,1) -- storage verifica transformado, quando = 1 player esta transformado.
