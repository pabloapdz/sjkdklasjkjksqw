...
local position2 = {x=getCreaturePosition(cid).x, y=getCreaturePosition(cid).y, z=getCreaturePosition(cid).z+0}      
  ...
  if os.time() - getPlayerStorageValue(cid, 55691) >= config.cooldown then
setPlayerStorageValue(cid, 55691, os.time())
...
doPlayerSendCancel(cid, "Sanzengarasu no Jutsu esta em cooldown, voce deve esperar "..(config.cooldown - (os.time() - getPlayerStorageValue(cid, 55691))).." segundos.")
return false
end  
    return true
end