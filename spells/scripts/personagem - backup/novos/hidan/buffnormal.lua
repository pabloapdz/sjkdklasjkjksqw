...
local effect = {488} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
local health = 2200 -- A cada 1 segundo quantos aumentar de vida
...
function magicEffectHD(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, 11223) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
if getPlayerStorageValue(cid, 11223) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    doCombat(cid, combat, var)
	doPlayerSendTextMessage(cid,27,'+UP Buff.') 
    doPlayerSay(cid, 'Im feeling better!', TALKTYPE_ORANGE_1)
    tempo2 = 0
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffectHD, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
    setPlayerStorageValue(cid, 11223,1) -- storage verifica transformado, quando = 1 player esta transformado.
