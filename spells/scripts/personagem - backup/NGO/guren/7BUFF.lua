...
local effect = {251} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function magicEffectGUREN(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, 100049) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
if getPlayerStorageValue(cid, 100049) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffectGUREN, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
